...
class MemoriesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create memory" do
    assert_difference("Memory.count") do
      post memories_url, params: { memory: { content: @memory.content, restaurant_id: @memory.restaurant_id, title: @memory.title, user_id: @memory.user_id } }
    end
  ...
  test "should update memory" do
    patch memory_url(@memory), params: { memory: { content: @memory.content, restaurant_id: @memory.restaurant_id, title: @memory.title, user_id: @memory.user_id } }
